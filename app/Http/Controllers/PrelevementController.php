...
class PrelevementController extends Controller
{
     ...
     public function create($id){
    	...
    	return view('prelevement.create', compact('lot'));
    }
     ...
     public function store(Request $request){
          ...
          $data["prelevement_dateEnr"] = date('Y-m-d H:i:s');
          $data["prelevement_enregistrerPar"] = session('AGENT_NUMERO');
          ...
          if($fichier){
            ...
            $data['prelevement_fichierJoint'] = $img_url;
        }
        else{
            $data['prelevement_fichierJoint'] = "uploads/docs/noimage.png";
        }

    	 $lot = DB::table('bp_lot_prelevement')->insert($data);

         $update = DB::table('dc_lots')->where('lot_num',$request->lot)->update(['lot_prelevement'=>'1']);
    ...
    public function edit($id){
        ...
        $bp = DB::table('bp_lot_prelevement')->where('bp_num',$id)->first();
    ...
    public function validatePrel(Request $request, $id)
    {
 
      $data["prelevement_numero"] = $id;
      $data["inter_service"] = session('AGENT_CATEGORIE');
      $data["prelev_operPar"] = session('AGENT_NUMERO');
      $data["prelev_operValiderDate"] = date('Y-m-d H:i:s');

      DB::table('bp_lot_prelevement_operation')->insert($data);

      $lot = DB::table('bp_lot_prelevement')->select('lot_numero')->where('prelevement_num',$id)->first();
